...
const Order = () =>{
  ...
  useEffect(() => {
    setDataOrder(StoreList.getDataOrder().sort((a, b) => (b.update_time - a.update_time)));
  },[]);
  
  const setSortTime = (sort) => {
    if(sort){
      setDataOrder(dataOrder.sort((a, b) => (a.update_time - b.update_time)));
    }else{
      setDataOrder(dataOrder.sort((a, b) => (b.update_time - a.update_time)));
    }
